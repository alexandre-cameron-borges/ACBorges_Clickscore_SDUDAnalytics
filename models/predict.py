...
def predict_cb(text: str, age_norm: float, gender_id: int) -> float:
    global _ctr_model, _tokenizer_cb
    if _ctr_model is None:
        ...
        _ctr_model = ClickbaitModel(n_genders=3)
        _ctr_model.load_state_dict(torch.load(path, map_location="cpu"))
        _ctr_model.eval()
    enc    = _tokenizer_cb(text, padding="max_length", truncation=True, max_length=128, return_tensors="pt")
    logits = _ctr_model(enc.input_ids, enc.attention_mask, torch.tensor(age_norm), torch.tensor(gender_id))
    return torch.sigmoid(logits).item()

class ClickbaitModel(nn.Module):
    def __init__(self, n_genders=3):
        super().__init__()
        self.bert       = BertModel.from_pretrained(CB_REPO, use_auth_token=HF_TOKEN)
        self.age_fc     = nn.Linear(1,16)
        self.gender_emb = nn.Embedding(n_genders,8)
        hid = self.bert.config.hidden_size + 16 + 8
        self.head = nn.Sequential(nn.Linear(hid,64), nn.ReLU(), nn.Dropout(0.1), nn.Linear(64,1))
    def forward(self, input_ids, attention_mask, age, gender):
        ...
        return self.head(torch.cat([out, a, g], dim=1)).squeeze(-1)
...
def predict_ctr(text: str) -> float:
    global _ctr_model2, _tokenizer_ctr
    if _ctr_model2 is None:
        ...
        _ctr_model2 = CTRModel()
        _ctr_model2.load_state_dict(torch.load(path, map_location="cpu"))
        _ctr_model2.eval()
    enc = _tokenizer_ctr(text, padding="max_length", truncation=True, max_length=128, return_tensors="pt")
    val = _ctr_model2(enc.input_ids, enc.attention_mask)
    return float(val.item())

class CTRModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.bert = BertModel.from_pretrained(CTR_REPO, use_auth_token=HF_TOKEN)
        hid = self.bert.config.hidden_size
        self.head = nn.Sequential(nn.Linear(hid,64), nn.ReLU(), nn.Dropout(0.1), nn.Linear(64,1))
    def forward(self, input_ids, attention_mask):
        return self.head(self.bert(input_ids, attention_mask).pooler_output).squeeze(-1)
